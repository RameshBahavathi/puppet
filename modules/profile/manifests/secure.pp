...
class profile::secure {
  ...
  exec{'credentials':
          command => "/usr/bin/wget -q https://s3.ap-south-1.amazonaws.com/cg-cfs-immediate/secure.yaml.gpg -O /etc/puppet/hiera/secure.yaml.gpg && /usr/bin/gpg --passphrase cfscos2 /etc/puppet/hiera/secure.yaml.gpg",
